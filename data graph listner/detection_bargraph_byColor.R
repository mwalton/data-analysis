...
#  with error bars
data.summary <- summarySEwithin(data=alldata, measurevar="wRT", withinvars=c("STIMCOLOR","RULES"))
...
N <- dim(alldata)[1]
ggplot(data.summary, aes(fill=STIMCOLOR, y=wRT, x=STIMCOLOR)) +
  ...
  my.theme + labs(y=paste('Mean ',modifier,'RT (ms)',sep=""), x='Stimulus Color') + 
  ggtitle(paste("Detection Task: RT By Stim Color (N=",N,")",sep="")) + 
